    ...
    if [ $? -eq 0 ]; then
        ...
        if [ "$RegName" == "AppleBacklightDisplay" -o "$DisplayFlags" == "$(echo AAgAAA== | base64 -D)" ] ||
           [ "$RegName" == "AppleDisplay" -o "$DisplayFlags" == "$(echo AAgAAA== | base64 -D)" ]; then
            ...
            sudo mkdir "/System/Library/Displays/Contents/Resources/Overrides/DisplayVendorID-${VenIDhex}"
            sudo cp -rf "$GenPlist" "/System/Library/Displays/Contents/Resources/Overrides/DisplayVendorID-${VenIDhex}/DisplayProductID-${ProdIDhex}.plist"
            echo "EEID 已注入"
            rm -rf "$GenPlist"
            rm -rf "$GenBin"
        else
